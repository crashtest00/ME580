...
from drone_game import drone_game
import time


def run_missions(train_indicator=0):  # 1 means Train, 0 means simply Run
    BUFFER_SIZE = 2000
    BATCH_SIZE = 32
    GAMMA = 0.99
    ...
    action_dim = 2  # size of action space
    state_dim = 5  # num of features in state
    ...
    # Wait until drone gets to altitude
    while not env.check_ai_nav():
        print("AI Nav Inactive.")
        time.sleep(2)
    print ("AI has been activated.")
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            if np.random.random() > epsilon:  # random returns random float between [0,1]
                ...
                a_type = "Explore"
                a_t = np.random.uniform(-0.1, 0.1, size=(1, action_dim))  # This needs tweaked to reflect the vels expected
